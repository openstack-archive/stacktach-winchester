...
class PipelineManager(object):
    ...
    def _log_statistics(self):
        ...
        self.trigger_manager.debugger.dump_debuggers()
    ...
    def fire_stream(self, stream, debugger):
        try:
    ...
    def expire_stream(self, stream, debugger):
        try:
    ...
    def process_ready_streams(self, batch_size, expire=False):
        ...
        for stream in streams:
            trigger_def = self.trigger_map.get(stream.name)
            debugger = self.trigger_manager.debugger.get_debugger(trigger_def)
            if expire:
                self.expire_stream(stream, debugger)
            else:
                self.fire_stream(stream, debugger)
