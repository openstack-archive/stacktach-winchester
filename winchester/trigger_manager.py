...
class TriggerManager(object):
    ...
    def config_description(cls):
        return dict(config_path=ConfigItem(
                    ...
                    # changed required to False
                    trigger_definitions=ConfigItem(required=False,
    ...
    def __init__(self, config, db=None, stackdistiller=None, trigger_defs=None,
        ...
        if stackdistiller is not None:
            ...
            # distiller_config should be optional
        ...
        if trigger_defs is not None:
            ...
            # trigger_definition config file is optional - can use methods to add, delete
    ...
    def add_trigger_definition(self, list_of_triggerdefs, debugger=None):
        ''' Pass in a list of triggerdef dictionaries to add '''
        if debugger is None:
            debugger = self.debug_manager            
        for td in list_of_triggerdefs:
            if self.trigger_map.has_key(td['name']) == False:
                # add if name is unique
    ...
    def delete_trigger_definition(self, trigger_def_name):
        ''' Pass in the stream name to delete. names are unique '''
        if self.trigger_map.has_key(trigger_def_name):
            self.trigger_definitions.remove(self.trigger_map.get(trigger_def_name))
